...
def text2image_ldm_stable(
    ...
    controller,
    num_inference_steps: int = 50,
    guidance_scale: float = 7.5,     #guidance_scale=7   它控制了无条件和有条件输入之间的平衡，较高的值会让生成更接近给定的文本提示。
    generator: Optional[torch.Generator] = None,
    latent: Optional[torch.FloatTensor] = None,    #可选的初始潜在向量。如果未提供，则会随机初始化。
    ...
    text_embeddings = model.text_encoder(text_input.input_ids.to(model.device))[0]    # torch.Size([1, 77, 768])
#     print(text_embeddings.shape)
    max_length = text_input.input_ids.shape[-1]
    uncond_input = model.tokenizer(
        [""] * batch_size, padding="max_length", max_length=max_length, return_tensors="pt"     # 无条件输入
    )
    uncond_embeddings = model.text_encoder(uncond_input.input_ids.to(model.device))[0]      # torch.Size([1, 77, 768])
    
    context = [uncond_embeddings, text_embeddings]    # len=2
    if not low_resource:
        context = torch.cat(context)           # torch.Size([2, 77, 768])
    latent, latents = init_latent(latent, model, height, width, generator, batch_size)   # torch.Size([1, 4, 64, 64])      torch.Size([1, 4, 64, 64]) 
    
    # set timesteps
    extra_set_kwargs = {"offset": 1}
    model.scheduler.set_timesteps(num_inference_steps)
    for t in tqdm(model.scheduler.timesteps):     # 会显示一张图片生成的进度条
        latents = diffusion_step(model, controller, latents, context, t, guidance_scale, low_resource)
    
    image = latent2image(model.vae, latents)   # (1, 512, 512, 3)       torch.Size([1, 4, 64, 64]) 
